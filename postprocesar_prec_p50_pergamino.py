# -*- coding: utf-8 -*-
"""Compatibilidad PREDWEEM — Pergamino.
...
def aplicar_p50_coherente(path: Path = ARCHIVO) -> bool:
    ...
    tipo = df.get(
        "TipoDato", pd.Series("", index=df.index)
    ).astype(str)
    ...
    for operativo, percentil, media in PARES:
        ...
        valores_p50 = pd.to_numeric(
            df.loc[mascara, percentil], errors="coerce"
        )
        if valores_p50.isna().any():
            raise ValueError(
                f"{percentil} contiene nulos; no se modificó {path}."
            )
        ...
        original = pd.to_numeric(
            df.loc[mascara, operativo], errors="coerce"
        )
