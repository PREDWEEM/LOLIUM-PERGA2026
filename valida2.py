...
# ---------------------------------------------------------
def optimizar_parametros_hidricos_2d(df_meteo, df_campo, modelo_ann, latitud_pergamino=-33.9443):
    df = df_meteo.copy()
...
dga_optimo = st.sidebar.number_input("Objetivo Control", value=800, step=10)
dga_critico = st.sidebar.number_input("Límite Ventana", value=900, step=10)
    ...
    if st.button("Ejecutar Barrido Hídrico"):
        if df_meteo_raw is not None and df_campo_raw is not None and modelo_ann is not None:
            with st.spinner('Ejecutando iteraciones hídricas puras por evento...'):
                ...
                tabla_optima = optimizar_parametros_hidricos_2d(df_meteo_opt, df_campo_opt, modelo_ann, latitud_pergamino=-33.9443)
