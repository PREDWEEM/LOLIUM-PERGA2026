# -*- coding: utf-8 -*-
"""Motor ecofisiológico y métricas event-to-event."""
from __future__ import annotations
from typing import Any, Mapping
import numpy as np
import pandas as pd
from .data import EPS, DEFAULT_WEIGHTS, default_parameters, prepare_weather


def calculate_et0_hargreaves(jday: np.ndarray, tmax: np.ndarray, tmin: np.ndarray, latitude: float = -33.9443) -> np.ndarray:
    ...
    acos_arg = np.clip(-np.tan(lat_rad) * np.tan(dec), -1.0, 1.0)
    ws = np.arccos(acos_arg)
...
def _apply_first_peak(values: np.ndarray, threshold: float, persistence: int) -> tuple[np.ndarray, int | None]:
    ...
    if persistence == 1:
        ...
        rolling = pd.Series(above.astype(int)).rolling(persistence, min_periods=persistence).sum().to_numpy()
...
def simulate_emergence(
    ...
    df["TMAX_suelo"] = df["Tmedia_aire"] + amplitude * float(p["mod_termico"])
    df["TMIN_suelo"] = df["Tmedia_aire"] - amplitude * float(p["mod_termico"])
    ...
    df["ET0"] = calculate_et0_hargreaves(
        df["Julian_days"].to_numpy(), df["TMAX"].to_numpy(), df["TMIN"].to_numpy(), latitude
    )
    df["W_superficial"] = surface_water_balance(
        df["Prec"].to_numpy(), df["ET0"].to_numpy(), float(p["w_max"]), float(p["ke_suelo"])
    )
    rel_water = df["W_superficial"].to_numpy(float) / max(float(p["w_max"]), EPS)
    df["Humedad_Relativa"] = rel_water
    exponent = np.clip(-float(p["pendiente_hidrica"]) * (rel_water - float(p["humedad_p50"])), -60, 60)
    ...
    # La recarga se define por el estado hídrico alcanzado, no por exigir un único
    # día de lluvia >= Wmax. Esto permite representar lluvias sucesivas moderadas.
...
def synchronize_intervals(simulation: pd.DataFrame, field: pd.DataFrame) -> pd.DataFrame:
    ...
    # El primer conteo también es un flujo válido. Se integra desde el comienzo
    # de la serie meteorológica hasta la primera fecha de muestreo. Esto evita
    # perder el primer pico y garantiza que los flujos relativos sumen uno.
...
def validation_metrics(sync: pd.DataFrame, detection_threshold: float = 0.05) -> dict[str, float | int]:
    zero = {
        "Pearson_Flujos": 0.0, "NSE_Flujos": -1.0, "KGE_Flujos": -1.0,
        "RMSE_Acumulado": 1.0, "CCC_Acumulado": 0.0, "R2_Acumulado": -1.0,
        "Exactitud": 0.0, "F1_Score": 0.0, "Desfase_T50": 365.0,
    ...
    obs_variance = np.sum((obs - np.mean(obs)) ** 2)
    nse = 1.0 - np.sum((sim - obs) ** 2) / obs_variance if obs_variance > EPS else -1.0
    if len(obs) >= 2 and np.mean(obs) > EPS and np.std(obs) > EPS:
    ...
    t50_lag = float((t50_sim.iloc[0] - t50_obs.iloc[0]).days) if not t50_obs.empty and not t50_sim.empty else 365.0

    values = {
        "Pearson_Flujos": pearson, "NSE_Flujos": float(nse), "KGE_Flujos": float(kge),
        "RMSE_Acumulado": rmse, "CCC_Acumulado": ccc, "R2_Acumulado": float(r2),
        "Exactitud": float(accuracy), "F1_Score": float(f1), "Desfase_T50": t50_lag,
        "Hits": hits, "Misses": misses, "Falsos_Positivos": false_pos,
...
def objective_score(metrics: Mapping[str, float | int], weights: Mapping[str, float] | None = None) -> float:
    ...
    return float(sum(max(0.0, float(w.get(k, 0.0))) * transformed[k] for k in transformed) / denominator)
