# -*- coding: utf-8 -*-
"""Validación cruzada temporal interna para un único conjunto de campo."""
...
DEFAULT_CV_WEIGHTS = {
    ...
    "F1_Score": 0.15,
    "RMSE_Acumulado": 0.15,
    # El T50 dentro de un bloque no equivale al T50 global de la campaña.
    "Desfase_T50": 0.00,
}
...
def build_interval_table(
    ...
    """Convierte cada conteo en un intervalo explícito inicio-fin."""
...
def synchronize_selected_intervals(
    simulation: pd.DataFrame,
    intervals: pd.DataFrame,
) -> pd.DataFrame:
    """Integra la simulación sólo en los intervalos retenidos."""
    ...
    for group, part in raw.groupby("Grupo", sort=False):
        ...
        part["Campo_Relativo"] = (
            part["Flujo_Obs_Abs"] / obs_total
            if obs_total > EPS
            else 0.0
        )
        part["Sim_Relativo"] = (
            part["Flujo_Sim_Abs"] / sim_total
            if sim_total > EPS
            else 0.0
        )
        part["Campo_Acumulado"] = (
            part["Acum_Obs_Abs"] / obs_total
            if obs_total > EPS
            else 0.0
        )
        part["Sim_Acumulado"] = (
            part["Acum_Sim_Abs"] / sim_total
            if sim_total > EPS
            else 0.0
        )
        frames.append(part)
...
def make_temporal_folds(
    ...
    """Divide cada serie en bloques cronológicos contiguos."""
    ...
    for group, part in intervals.groupby("Grupo", sort=False):
        ...
        if folds_for_group < 2:
            raise ValueError(
                f"El grupo {group} requiere al menos "
                f"{2 * min_intervals_per_fold} fechas para CV temporal."
            )
        chunks = np.array_split(np.arange(len(part)), folds_for_group)
        for fold_number, indices in enumerate(chunks, start=1):
...
def evaluate_candidate_temporal_cv(
    ...
    summary = {
        "Score_CV": float(
            np.mean(scores)
            - float(robustness_penalty) * np.std(scores)
        ),
    ...
    for metric in [
        ...
        summary[f"{metric}_Media"] = (
            float(fold_df[metric].mean())
            if not fold_df.empty
            else np.nan
        )
    sync_all = (
        pd.concat(sync_frames, ignore_index=True)
        if sync_frames
        else pd.DataFrame()
    )
...
def optimize_parameters_temporal_cv(
    ...
    *,
    ...
    """Selecciona parámetros con validación interna por bloques temporales."""
    unknown = [
        name for name in optimized_parameters
    ...
    seeds = global_df.head(max(1, int(top_seeds)))[
    ...
    results = results.drop_duplicates(
        subset=list(PARAMETER_SPACE),
        keep="first",
    )
    results = results.sort_values(
        ["Score_CV", "Score_CV_Peor_Bloque"],
        ascending=False,
    ...
    for name, spec in PARAMETER_SPACE.items():
        best_params[name] = (
            int(best_params[name])
            if spec.integer
            else float(best_params[name])
        )
