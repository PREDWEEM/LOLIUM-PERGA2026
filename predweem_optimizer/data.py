# -*- coding: utf-8 -*-
"""Motor de calibración y validación ecofisiológica para PREDWEEM.

El módulo separa estrictamente:
1. calibración: usada para seleccionar parámetros;
2. validación independiente: usada una sola vez, después de elegir el óptimo.

No depende de Streamlit y puede probarse desde Python o integrarse en otras apps.
"""
...
PARAMETER_SPACE: dict[str, ParameterSpec] = {
    ...
    "ke_suelo": ParameterSpec(0.10, 0.95, 0.25),
...
def default_parameters() -> dict[str, float | int]:
    ...
    for name, spec in PARAMETER_SPACE.items():
        result[name] = int(spec.default) if spec.integer else float(spec.default)
    return result
...
def prepare_weather(df: pd.DataFrame) -> pd.DataFrame:
    ...
    missing = [name for name, col in {"Fecha": date_col, "TMAX": tmax_col, "TMIN": tmin_col, "Prec": prec_col}.items() if col is None]
    if missing:
...
def prepare_field(
    ...
    """Estandariza observaciones de campo.

    value_mode:
      - interval: conteo/flujo observado desde el muestreo anterior;
    ...
    if value_column is None:
        numeric_candidates = [c for c in df.columns if c != date_column and pd.to_numeric(df[c], errors="coerce").notna().sum() >= 2]
    ...
    out["Grupo"] = df[group_column].astype(str) if group_column else "Conjunto_1"
    ...
    for group, part in out.groupby("Grupo", sort=False):
        ...
        if mode == "cumulative":
            ...
            flows = np.diff(np.r_[0.0, values])
            part["Observado"] = np.clip(flows, 0.0, None)
        else:
    ...
    if len(result) < 3:
        raise ValueError("Se requieren al menos 3 fechas de muestreo de campo.")
