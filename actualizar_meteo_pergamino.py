# -*- coding: utf-8 -*-
# ===============================================================
# 🌾 NODO CLIMÁTICO PREDWEEM — PERGAMINO
#
# Serie operativa:
...
LATITUD = float(os.getenv("LATITUD", "-33.9443"))
LONGITUD = float(os.getenv("LONGITUD", "-60.5745"))
...
ARCHIVO_SIGA_CACHE = Path("data/siga_pergamino_observado.csv")
DIRECTORIO_PRONOSTICOS = Path("data/historico_pronosticos")
ARCHIVO_ESTADO = Path("data/estado_actualizacion_meteo.json")

SIGA_ARCHIVO_LOCAL = Path(os.getenv("SIGA_LOCAL_FILE", "A872814.xls"))
...
def buscar_archivo_siga_local(archivo_preferido: Path | None = None) -> Path | None:
    ...
    for patron in ("A*.xls", "A*.xlsx", "*siga*.xls", "*siga*.xlsx", "*siga*.csv"):
...
def descargar_siga(fecha_inicio: date, fecha_fin: date) -> tuple[bytes, str, str]:
    ...
    nombre = coincidencia.group(1) if coincidencia else Path(url).name or "siga_pergamino_descarga.xls"
...
def normalizar_dataframe_siga(tabla: pd.DataFrame, fecha_limite_exclusiva: date) -> pd.DataFrame:
    ...
    salida["Fuente"] = "SIGA_INTA_PERGAMINO"
...
def obtener_siga_dataframe(fecha_inicio: date, fecha_fin: date, archivo_forzado: Path | None = None) -> tuple[pd.DataFrame, str]:
    ...
    if SIGA_URL_TEMPLATE and archivo_forzado is None:
        try:
            print("📡 Descargando observaciones diarias SIGA Pergamino...")
...
def cargar_pronostico_ecmwf() -> pd.DataFrame:
    ...
    archivo = DIRECTORIO_PRONOSTICOS / f"ecmwf_ifs_ens_025_pergamino_{marca}.csv"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Actualiza meteo_daily.csv para PREDWEEM Pergamino.")
