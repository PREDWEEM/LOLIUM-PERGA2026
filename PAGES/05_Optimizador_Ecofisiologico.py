...
APP_VERSION = "UNICO_SET_CV_TEMPORAL_v2_2026-07-13"
...
st.title("🧬 Optimizador ecofisiológico — ÚNICO SET DE CAMPO")
st.caption(
    f"PREDWEEM Pergamino 2026 · CV temporal interna · {APP_VERSION}"
)
st.warning(
...
run = st.button(
    "🚀 Optimizar ÚNICO SET: VALIDA.xlsx",
    type="primary",
    width="stretch",
)
...
if "eco_cv_result" in st.session_state:
    ...
    metric_cards(
        "Evaluación descriptiva sobre toda la serie (mismo set)",
        apparent_for_cards,
        "Score_Aparente",
    )
    ...
    with tabs[3]:
        ...
        if sync.empty:
            ...
            fig.update_layout(
                title=(
                    "Evaluación completa del mismo set "
