# -*- coding: utf-8 -*-
# ===============================================================
# 👑 PREDWEEM INTEGRAL vK4.9.15 VISUAL V3 — LOLIUM PERGAMINO 2026
...
# - OPTIMIZADOR 2D: Barrido de W_Max y cobertura (Ke derivado físicamente) adaptado a ventanas reales de campo.
...
# - SIMULACIÓN: Sin incremento térmico artificial; lag fijo de emergencia de +15 días.
# ===============================================================
...
LAG_EMERGENCIA_DIAS = 15  # Lag fijo: retrasa 15 días la emergencia simulada
...
def simular_emergencia_local(
    ...
    df = aplicar_lag_emergencia(
        df, lag_dias=int(lag_emergencia_dias), col="EMERREL"
    )
...
# ---------------------------------------------------------
# 3.5 MÓDULO OPTIMIZADOR 2D (CALIBRACIÓN SUELO POR EVENTO)
# ---------------------------------------------------------
...
st.caption("Pergamino · VISUAL V3 · ventana eficiente verde · exportación PNG de alta resolución · lag fijo 22 días")
...
with st.expander("📂 1. Datos del Lote", expanded=True):
    ...
    with col_rastrojo:
        with st.container(border=True):
            ...
            cobertura_pct = st.slider("Cobertura de Rastrojo en Suelo (%)", min_value=0, max_value=100, value=80, step=5)
...
with st.sidebar.expander("🛠️ Modo Dev: Calibrador Bio-Físico 2D", expanded=False):
    st.caption("Mapea combinaciones hídricas óptimas directamente contra los intervalos reales de tus datos de campo.")
...
if df_meteo_raw is not None and modelo_ann is not None:
    ...
    st.plotly_chart(go.Figure(data=go.Heatmap(z=[df["EMERREL"].values], x=df["Fecha"], y=["Emergencia"], colorscale=colorscale_hard, zmin=0, zmax=1, showscale=False)).update_layout(height=120, margin=dict(t=30, b=0, l=10, r=10), title="Mapa de Riesgo Temporal (Tasa Diaria Pergamino)"), width="stretch")
            ...
            st.plotly_chart(
                ...
                config={
                    ...
                    "toImageButtonOptions": {
                        "format": "png",
                        "filename": "PREDWEEM_Pergamino_dinamica_emergencia",
                        "height": 1000,
                        "width": 2000,
                        "scale": 2,
                    },
